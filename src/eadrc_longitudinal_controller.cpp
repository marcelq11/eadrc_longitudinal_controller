  ...
  diagnostic_updater_(&node)
{
  ...
  // parameters for ESO
  p_obserwer = std::make_shared<eadrc_longitudinal_controller::ESO>(0.1, 6);
  m_kp=2;
  m_b_hat=1;
  ...
  m_set_param_res = node.add_on_set_parameters_callback(
    std::bind(&EadrcLongitudinalController::paramCallback, this, _1));
...
rcl_interfaces::msg::SetParametersResult EadrcLongitudinalController::paramCallback(
...
EadrcLongitudinalController::calcInterpolatedTargetValue(
...
double EadrcLongitudinalController::predictedVelocityInTargetPoint(
...
{
  if(p_obserwer == nullptr)
  {
    RCLCPP_ERROR_THROTTLE(logger_, *clock_, 3000, "Obserwer is not set_______________________________________________________________________");
  }
  Eigen::Vector2d stateVector = p_obserwer->getStateVector();
...
double EadrcLongitudinalController::applyVelocityFeedback(
    const Motion target_motion, const double dt, const double current_vel, const Shift & shift)
{
  ...
  //   m_pid_vel.calculate(error_vel_filtered, dt, enable_integration, pid_contributions);//tu oblicznay jest pid

  double eadrc_acc = calculateControlSignal(error_vel_filtered, dt);
  ...
  const double feedback_acc = eadrc_acc; //ZMIENIC
...
void EadrcLongitudinalController::updateDebugVelAcc(
...
void EadrcLongitudinalController::setupDiagnosticUpdater()
{
  diagnostic_updater_.setHardwareID("Eadrc_longitudinal_controller");
  diagnostic_updater_.add("control_state", this, &EadrcLongitudinalController::checkControlState);
}

void EadrcLongitudinalController::checkControlState(
